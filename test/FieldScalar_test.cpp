...
int main() {
	...
	FieldScalar<Complex> fs1(lat.getVol(), &rndGen, oneInit);
	for( size_t x = 0; x < lat.getVol(); x++ ) {
	...
	FieldScalar<Complex> fs1copy( fs1 );
	for( size_t x = 0; x < lat.getVol(); x++ ) {
	...
	FieldScalar<Complex> fsSum(lat.getVol(), &rndGen, randomInit);
	fsSum = fs1+fs1copy;
	for( size_t x = 0; x < lat.getVol(); x++ ) {
		if( Complex(2.,0) !=fsSum(x) ) correct = false;
	}
	if( !correct ) {
		std::cout << "Copy assignement or sum failed! Field has values:" << std::endl;
		fsSum.Print();
		exit(1);
	} else {
	...
	FieldScalar<Complex> fsrnd(latLarge.getVol(), &rndGen, randomInit);
	...
	FieldScalar<Real> fsGauss(latLarge.getVol(), &rndGen, gaussianInit);
	...
	FieldScalar<Complex> fsGaussC(latLarge.getVol(), &rndGen, gaussianInit);
